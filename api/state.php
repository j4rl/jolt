...
$token = $_GET['token'] ?? '';
$s = db()->prepare('SELECT g.*,q.title,(SELECT COUNT(*) FROM questions WHERE quiz_id=g.quiz_id) total_questions FROM games g JOIN quizzes q ON q.id=g.quiz_id WHERE g.id=?');
...
$players = db()->query('SELECT id,name,avatar,score,joined_at FROM players WHERE game_id='.$gameId.' ORDER BY score DESC,joined_at ASC')->fetch_all(MYSQLI_ASSOC);

if ($g['status'] === 'question' || $g['status'] === 'results') {
    $s = db()->prepare('SELECT * FROM questions WHERE quiz_id=? ORDER BY position LIMIT ?,1');
    $idx = max(0, (int)$g['current_question'] - 1);
    $s->bind_param('ii', $g['quiz_id'], $idx);
    $s->execute();
    $q = $s->get_result()->fetch_assoc();
    if ($q) {
        $answers = db()->query('SELECT id,text'.($g['status'] === 'results' ? ',is_correct' : '').' FROM answers WHERE question_id='.(int)$q['id'].' ORDER BY position')->fetch_all(MYSQLI_ASSOC);
        $elapsed = $g['question_started_at'] ? (int)((microtime(true) - strtotime($g['question_started_at'])) * 1000) : 0;
        $count = (int)db()->query('SELECT COUNT(*) c FROM responses WHERE game_id='.$gameId.' AND question_id='.(int)$q['id'])->fetch_assoc()['c'];
        ...
        if ($g['status'] === 'results') {
            $points = [];
            $result = db()->query('SELECT player_id,points_awarded FROM responses WHERE game_id='.$gameId.' AND question_id='.(int)$q['id']);
...
if ($token) {
    $s = db()->prepare('SELECT id,score FROM players WHERE token=? AND game_id=?');
    ...
    if (isset($data['question'])) {
        $s = db()->prepare('SELECT is_correct,points_awarded FROM responses WHERE player_id=? AND question_id=?');
